...
import com.veltpvp.nirvana.packet.party.PartyDisbandPacket;
...
public class PartyDisbandCommand extends BasePartyCommand {

    @Command(name = "party.disband", aliases = {"p.disband", "t.disband", "team.disband"})
    public void onCommand(CommandArgs command) {
        final Player player = command.getPlayer();
        main.getNetwork().sendPacket(new PartyDisbandPacket(new PartyMember(player.getName(), player.getUniqueId())), NirvanaChannels.APPLICATION_CHANNEL, RabbitNetworkDeliveryType.DIRECT, new PacketResponseHandler() {
