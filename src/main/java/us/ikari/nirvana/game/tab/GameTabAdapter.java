package us.ikari.nirvana.game.tab;
...
public class GameTabAdapter implements TabAdapter {
    ...
    public TabTemplate getTemplate(Player player) {
        ...
        if (game.getState() == GameState.PLAY) {
            for (String line : main.getLangFile().getStringList("TAB.GAME.LEFT", LanguageConfigurationFileLocale.EXPLICIT, player.getStatistic(Statistic.PLAYER_KILLS), gamePlayer != null ? gamePlayer.getKitName() : "N/A", game.getMap(), player.getLocation().getBlockX(), player.getLocation().getBlockZ(), getCardinalDirection(player), getPlayers(), game.getAlivePlayers().size(), getSpectators(), "SW-1", game.getGameTime().secondsLeft(main.getConfigFile().getInteger("STATE.LOBBY.COUNTDOWN")) + 1)) {
                toReturn.left(line);
            }
            for (String line : main.getLangFile().getStringList("TAB.GAME.MIDDLE", LanguageConfigurationFileLocale.EXPLICIT, player.getStatistic(Statistic.PLAYER_KILLS), gamePlayer != null ? gamePlayer.getKitName() : "N/A", game.getMap(), player.getLocation().getBlockX(), player.getLocation().getBlockZ(), getCardinalDirection(player), getPlayers(), game.getAlivePlayers().size(), getSpectators(), "SW-1", game.getGameTime().secondsLeft(main.getConfigFile().getInteger("STATE.LOBBY.COUNTDOWN")) + 1)) {
                toReturn.middle(line);
            }
    ...
    private List<String> getPlayers() {
        ...
        for (GamePlayer gamePlayer : game.getPlayers()) {
            if (gamePlayer.getData().alive()) {
                toReturn.add(main.getLangFile().getString("TAB.ALIVE", LanguageConfigurationFileLocale.EXPLICIT) + gamePlayer.getName());
            } else {
                toReturn.add(main.getLangFile().getString("TAB.DEAD", LanguageConfigurationFileLocale.EXPLICIT) + gamePlayer.getName());
            }
        }
